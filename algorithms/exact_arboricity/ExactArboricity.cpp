...
ArboricityOutput ExactArboricity::compute(
    ...
    LogFn log)
{
    ...
    lg("Nodes: " + std::to_string(n) + " | Edges: " + std::to_string(m));
    lg("Beginning Nash-Williams Forest Cover Search...");

    while (low <= high) {
        int k = low + (high - low) / 2;
        lg("Testing K=" + std::to_string(k) + " (Forest limit)");
        ...
        if (std::abs(max_flow - m) < 1e-7) {
            lg(" -> SUFFICIENT (k=" + std::to_string(k) + ")");
            best_k = k;
            high = k - 1;
        } else {
            lg(" -> INSUFFICIENT (k=" + std::to_string(k) + ")");
            low = k + 1;
        }
    }
